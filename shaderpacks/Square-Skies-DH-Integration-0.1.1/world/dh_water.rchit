#version 460
#ifndef DH_WATER_GUIDE_ALPHA
#define DH_WATER_GUIDE_ALPHA 1
#endif
...
void sampleSurfaceState(bool useTexture,
    ...
    if (useWaterMaterial) {
        ...
        if (DH_WATER_GUIDE_ALPHA != 0) albedoValue.a = 1.0;
        tint = waterTint;
        mat.f0 = vec3(0.02);
    ...
    // v0.3: use a world-space optical ripple on the existing planar mesh.
    // This bypasses the legacy FFT normal/displacement path and UV tangents.
    if (useWaterMaterial && abs(baseGeoNormal.y) > 0.75) {
        float footprint = max(mainRay.coneWidth + gl_HitTEXT * mainRay.coneSpread,
                              length(worldPos) * 0.0008) / max(abs(viewDir.y), 0.15);
        SsWaterWave wave = ssWaterWave((worldPos + vec3(worldUBO.cameraPos.xyz)).xz, worldUBO.gameTime, footprint);
        vec3 up = normalize(vec3(-wave.slope.x, 1.0, -wave.slope.y));
        float grazing = smoothstep(0.0, 0.12, abs(viewDir.y));
        up = normalize(mix(vec3(0.0, 1.0, 0.0), up, grazing));
        // Limit the optical tilt near grazing views so reflection rays stay
        // above the actual flat surface instead of hitting submerged geometry.
        float maxTilt = abs(viewDir.y) * 0.4;
        up.xz *= min(1.0, maxTilt / max(length(up.xz), 1e-5));
        up = normalize(up);
        shadingNormal = baseGeoNormal.y >= 0.0 ? up : -up;
        mat.normal = vec3(0.0, 0.0, 1.0);
        mat.roughness = clamp(sqrt(0.018 * 0.018 + wave.unresolvedVariance * 0.4), 0.018, 0.12);
    }
...
vec3 sampleSurfaceDirectLight(SampledSurface surface,
    ...
    shadowRay.pad0 = 0u;
    shadowRay.ss5Mode = 0u;
    shadowRay.ss5WaterT = 1e20;
...
void main() {
    ...
    vec3 baseGeoNormal =
        normalizeF(normalMatrix * (baryCoords.x*m0.norm + baryCoords.y*m1.norm + baryCoords.z*m2.norm), vec3(0.0,1.0,0.0));
    vec3 planeGeoNormal = baseGeoNormal;
    vec3 dPduWorld = objectToWorld * dposdu;
    vec3 dPdvWorld = objectToWorld * dposdv;
    bool isWaterMaterial = true; // Native WATER-only SBT group
    bool hasFftWaterSurface = false;
    bool parallaxEnabled = VPT_ENABLE_PARALLAX != 0;
    bool useRealisticWaterSurface = false; // Square optical waves below; no atlas-driven LOD displacement
    ...
    if (useTexture) {
        ...
        if (textureMap.flag >= 0) {
            ivec4 flags = ivec4(round(sampleTexture(textures[nonuniformEXT(textureMap.flag)], textureUV, 0.0, false) * 255.0));
    ...
    rayStoreAux(mainRay, isWaterMaterial ? vec2(VPT_FFT_WATER_ORIGIN_BIAS, dot(gl_WorldRayDirectionEXT, planeGeoNormal) < 0.0 ? 3.0 : 4.0) : vec2(0.0));
    ...
    for (int localBounce = 0; localBounce < 1; ++localBounce) {
        ...
        if (isWaterMaterial && currentSurface.mat.transmission > EPS) {
            ...
            vec3 waterUpNormal = planeGeoNormal; // true DH outward face normal
