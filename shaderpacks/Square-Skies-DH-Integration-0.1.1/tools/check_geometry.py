...
SIZE=48.; BASE=224.; TOP=252.; FAR=4096.; EPS=.002
def hash2(x,z):
...
def box(x,z):
    ...
    threshold=.85+(.15-.85)*.55
    if field<=threshold:return None
    tier=math.floor(min(max((field-threshold)*8,0),2.999))
    return (x*SIZE,BASE,z*SIZE),((x+1)*SIZE,BASE+28*(.5+tier*.25),(z+1)*SIZE)
...
def dda(ro,rd):
    ...
    assert steps<256, 'Traversal exceeded reference budget'
    return total
def brute(ro,rd):
    span=layer(ro,rd)
    if span is None:return 0
...
rays=[((0,64,0),(0,1,0)),((0,400,0),(0,-1,0)),((0,64,0),(1,0,0)),
      ((0,230,0),(1,0,0)),((-48,230,-48),(-1,0,0)),((48,230,48),(0,0,-1)),
      ((-48,224,-48),(0,1,0)),((0,252,0),(0,-1,0))]
for _ in range(180):
    ro=(rng.uniform(-600,600),rng.choice([64,230,400]),rng.uniform(-600,600))
...
target=Path(__file__).parent/'validation/geometry-report.json'
