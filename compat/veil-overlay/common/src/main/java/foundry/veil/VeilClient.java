...
public class VeilClient {
    ...
    public static void init() {
        if (VeilBackend.hasOpenGlRenderer()) {
            ...
            Veil.LOGGER.warn("Veil Radiance backend selected. CPU services retained; unavailable render features: {}",
                    VeilBackend.unavailableFeatures());
            QuasarParticles.bootstrap();
            FlareManager.bootstrap();
            VeilEventPlatform.INSTANCE.onFreeNativeResources(RESOURCE_MANAGER::free);
        }
...
//            glEnable(GL_DEPTH_CLAMP); // TODO add config option
    ...
    // Keep registry schemas and factories for dependent mods/data; do not initialize GPU instances.
