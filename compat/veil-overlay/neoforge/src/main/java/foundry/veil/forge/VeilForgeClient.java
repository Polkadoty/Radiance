...
public class VeilForgeClient {
    ...
    public VeilForgeClient(IEventBus modEventBus) {
        if (!VeilBackend.hasOpenGlRenderer() && !Veil.platform().isModLoaded("radiance")) {
            throw new IllegalStateException("veil.backend=radiance requires the Radiance mod and its explicit rendering bridge");
        }
        VeilClient.init();
    ...
    private static void registerListeners(RegisterClientReloadListenersEvent event) {
        if (VeilBackend.hasOpenGlRenderer()) VeilRenderSystem.init();
        VeilReloadListeners.registerListeners((type, id, listener) -> event.registerReloadListener(listener));
        if (!VeilBackend.hasOpenGlRenderer()) return;
    ...
    private static void registerKeys(RegisterKeyMappingsEvent event) {
        if (VeilBackend.hasOpenGlRenderer() && Veil.IMGUIMC) {
