...
public class VeilForgeClientEvents {
    ...
    public static void clientDisconnected(ClientPlayerNetworkEvent.LoggingOut event) {
        if (VeilBackend.hasOpenGlRenderer()) VeilRenderSystem.renderer().getLightRenderer().free();
        VeilClientServerFlags.onDisconnect();
    }

    @SubscribeEvent
    public static void keyPressed(InputEvent.Key event) {
        if (VeilBackend.hasOpenGlRenderer() && Veil.IMGUIMC && event.getAction() == GLFW_PRESS && VeilImGuiCompat.EDITOR_KEY.matches(event.getKey(), event.getScanCode())) {
    ...
    public static void mousePressed(InputEvent.MouseButton.Pre event) {
        if (VeilBackend.hasOpenGlRenderer() && Veil.IMGUIMC && event.getAction() == GLFW_PRESS && VeilImGuiCompat.EDITOR_KEY.matchesMouse(event.getButton())) {
            VeilRenderSystem.renderer().getEditorManager().toggle();
        }
    }
    ...
    public static void registerClientCommands(RegisterClientCommandsEvent event) {
        ...
        if (!VeilBackend.hasOpenGlRenderer()) {
            dispatcher.register(Commands.literal("veilc").then(Commands.literal("backend").executes(ctx -> {
                ctx.getSource().sendSuccess(() -> Component.literal("Veil backend: Radiance. Unavailable: "
                        + VeilBackend.unavailableFeatures()), false);
                return 1;
            })));
            dispatcher.register(Commands.literal("quasar").executes(ctx -> {
                ctx.getSource().sendFailure(Component.literal("Quasar rendering is not ported to the Radiance backend."));
                return 0;
            }));
            return;
        }
